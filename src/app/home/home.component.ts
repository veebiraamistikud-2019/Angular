...
import { BestScoreManager } from './storage.service';
...
export class HomeComponent {
 ...
 setColors(col: number, row: number): string {
   if (this.isGameOver) {
     ...
     return COLORS.BODY;
   } else if (this.default_mode === 'classic' && this.checkObstacles(row, col)) {
 ...
 addListelem(): void{
  ...
  let listElemText = document.createTextNode(this.best_score)
  let listElemText2 = document.createTextNode(" points")
  listElem.appendChild(listElemText)
  listElem.appendChild(listElemText2)
  document.getElementById("last-score").appendChild(listElem);
 }
